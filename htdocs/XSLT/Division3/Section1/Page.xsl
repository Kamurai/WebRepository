...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="GetTitle">
		...
		<xsl:variable name="vDefault" select="'Antarrea'"/>
		
		<title>
			<xsl:choose>
				<xsl:when test="$vPage &lt;= 0"><xsl:value-of select="$vDefault"/></xsl:when>
				<xsl:when test="$vPage = 1">Revolutions: Invading Nations</xsl:when>
				<xsl:when test="$vPage = 2">Revolutions: Rebellion Against the Fist</xsl:when>
				<xsl:when test="$vPage = 3">Revolutions: Return to Arms</xsl:when>
				<xsl:when test="$vPage = 4">Revolutions: Post Wars</xsl:when>
				<xsl:when test="$vPage = 5">Avia: Elemental Angel</xsl:when>
