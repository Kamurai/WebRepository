...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="GetTitle">
		...
		<xsl:variable name="vDefault" select="'Digital Board Games'"/>
		
		<title>
			<xsl:choose>
				<xsl:when test="$vPage &lt;= 0"><xsl:value-of select="$vDefault"/></xsl:when>
				<xsl:when test="$vPage = 1">Kingdoms</xsl:when>
				<xsl:when test="$vPage = 2">Goblins and Guardians</xsl:when>
				<xsl:when test="$vPage = 3">MegaFleets</xsl:when>
