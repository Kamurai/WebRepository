...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="GetTitle">
		...
		<xsl:variable name="vDefault" select="'HTKB Home Page'"/>
		
		<title>
			<xsl:choose>
				<xsl:when test="$vPage &lt;= 0"><xsl:value-of select="$vDefault"/></xsl:when>
				<xsl:when test="$vPage = 1">About Us</xsl:when>
				<xsl:when test="$vPage = 2">Media</xsl:when>
				<xsl:when test="$vPage = 3">Git Hubs</xsl:when>
				<xsl:when test="$vPage = 4">Web Apps</xsl:when>
